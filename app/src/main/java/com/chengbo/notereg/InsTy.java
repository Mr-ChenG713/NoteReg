...
public class InsTy extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {

    private static final int ID_CURSOR_LOADER_TIPO = 0;

    private EditText editTextTipoNome;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editTextTipoNome = (EditText) findViewById(R.id.InsTypes);
    ...
    private void save() {
        ...
        String conteudoTipo = editTextTipoNome.getText().toString();

        if (conteudoTipo.trim().isEmpty()) {
            editTextTipoNome.setError(getString(R.string.title_activity_ins_ty));
            return;
        }
        ...
        try {
            getContentResolver().insert(NoteRegContentProvider.ENDERECO_TIPO, tipo.getContentValues());

            Toast.makeText(this, (R.string.State_s_Inser), Toast.LENGTH_SHORT).show();
            finish();

        } catch (Exception e) {
            Snackbar.make(editTextTipoNome, "Error while saving!", Snackbar.LENGTH_LONG).show();
            e.printStackTrace();
        }
    }
